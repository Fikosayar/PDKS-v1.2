...
// Replace the entire initDb function with one that matches Drizzle schema
const newInitDb = `
// --- INITIALIZE DATABASE ---
const initDb = async () => {
  try {
...
// Replace old initDb
c = c.replace(
  /\/\/ --- INITIALIZE DATABASE ---[\s\S]*?initDb\(\);/,
  newInitDb.trim()
);

fs.writeFileSync('server.ts', c, 'utf8');
console.log('initDb replaced with Drizzle-compatible schema');
