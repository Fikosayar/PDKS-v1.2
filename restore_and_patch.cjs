...
function applyAppPatches() {
  ...
  // handleScanSuccess
  c = c.replace(
    /const newDocRef = await addDoc\(collection\(db, 'attendance'\), \{\s*\.\.\.logPayload,\s*timestamp: serverTimestamp\(\),\s*\}\);/g,
  ...
  // Firebase imports
  c = c.replace(/^import \{.*?\} from 'firebase\/(firestore|auth)';\r?\n/g, '');
  c = c.replace(/^import \{.*?\} from '\.\.?\/lib\/firebase';\r?\n/g, '');
...
function applyServerPatches() {
  ...
  // Add the routes block after `app.post('/api/attendance'`
  // For simplicity, we just append it before `if (process.env.NODE_ENV !== 'production')`
  const missingRoutes = `
...
`;
  ...
  if (splitContent.length === 2) {
    ...
    // Replace the corrupted U+FFFD with properly encoded 'Sistem hatası' etc.
    serverCode = serverCode.replace(/Sistem hatas/g, 'Sistem hatası');
    serverCode = serverCode.replace(/Hatal/g, 'Hatalı');
    serverCode = serverCode.replace(/Geersiz giri/g, 'Geçersiz giriş');
    serverCode = serverCode.replace(/ok/g, 'Çok');
    serverCode = serverCode.replace(/devre d/g, 'devre dışı');
    serverCode = serverCode.replace(/irket bulunamad/g, 'Şirket bulunamadı');
