...
async function startServer() {
  ...
  app.post('/api/login', async (req, res) => {
    ...
    if (!checkRateLimit(clientIp)) return res.status(429).json({ error: '�ok fazla deneme.' });

    try {
      const userList = await db.select().from(users).where(eq(users.personnelId, personnelId)).limit(1);
      if (userList.length === 0) return res.status(401).json({ error: 'Hatal� ID veya �ifre' });
      const user = userList[0];

      const passwordMatch = await bcrypt.compare(password, user.passwordHash);
      if (!passwordMatch) return res.status(401).json({ error: 'Hatal� ID veya �ifre' });
      if (!user.isActive) return res.status(403).json({ error: 'Hesap devre d���' });
      ...
      res.status(500).json({ error: 'Sistem hatas�' });
    }
  });
